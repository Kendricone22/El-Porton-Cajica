...
const MENU = [
  ...
  { id:"h-sencilla", cat:"hamburguesas", combo:true, emoji:"🍔", img:"assets/Platos/hamburguesa-sencilla.jpg",
  ...
  { id:"h-sencilla-doble", cat:"hamburguesas", combo:true, emoji:"🍔", img:"assets/Platos/hamburguesa-sencilla-doble.jpg",
  ...
  { id:"h-clasica-doble", cat:"hamburguesas", combo:true, emoji:"🍔", img:"assets/Platos/hamburguesa-clasica-doble.jpg",
  ...
  { id:"h-mixta", cat:"hamburguesas", combo:true, emoji:"🍔", img:"assets/Platos/hamburguesa-mixta.jpg",
  ...
  { id:"h-campesina", cat:"hamburguesas", combo:true, emoji:"🍔", img:"assets/Platos/hamburguesa-campesina.jpg",
    ...
    options:[{label:"Koller",price:23000},{label:"Pollo",price:23500},{label:"Artesanal",price:27000}] },
  { id:"h-ranchera", cat:"hamburguesas", combo:true, emoji:"🍔", img:"assets/Platos/hamburguesa-ranchera.jpg",
  ...
  { id:"p-mexicano", cat:"perros", combo:true, emoji:"🌭",
    ...
    options:[{label:"Porción",price:19000}] },
  { id:"p-hawaiano", cat:"perros", combo:true, emoji:"🌭", img:"assets/Platos/perro-hawaiano.jpg",
...
/* =============================================================
 * EXPORTACIONES — única diferencia con js/data.js del sitio v1.
 * Todo lo de arriba es copia BYTE A BYTE: el menú jamás se
 * vuelve a escribir a mano (regla anti-invención del proyecto).
 *
 * Las tres constantes críticas para el precio se exportan
 * ANOTADAS con su tipo. Eso hace que TypeScript compare los
 * datos reales contra el tipo al compilar: si un producto se
 * queda sin `options`, o una adición apunta a una categoría que
 * no existe, falla el build en vez de fallar el cobro.
 * ============================================================= */
import type { ProductoMenu, Adicion, Categoria } from '@/types/menu';

/* -------------------------------------------------------------
 * Por qué hay casts aquí:
 * TypeScript deduce `cat: string` (no la unión de 7 categorías)
 * porque un array `const` es mutable y no puede garantizar que el
 * valor siga siendo el literal. Los datos SÍ son correctos; lo que
 * falla es la demostración.
 *
 * Un cast a secas sería una promesa vacía: taparía también los
 * errores de verdad. Así que debajo se COMPRUEBA lo prometido.
 * Si algún dato deja de cumplirlo, el build revienta aquí — que es
 * exactamente donde queremos enterarnos, y no en el momento de cobrar.
 * ----------------------------------------------------------- */
const CATEGORIES_TIPADAS = CATEGORIES as Categoria[];
const MENU_TIPADO = MENU as ProductoMenu[];
const ADICIONES_TIPADAS = ADICIONES as Adicion[];

const CATS_VALIDAS = new Set<string>(CATEGORIES_TIPADAS.map((c) => c.key));

for (const p of MENU_TIPADO) {
  if (!CATS_VALIDAS.has(p.cat)) {
    throw new Error(`[menu] El producto "${p.id}" tiene la categoría "${p.cat}", que no existe en CATEGORIES.`);
  }
  if (!Array.isArray(p.options) || p.options.length === 0) {
    throw new Error(`[menu] El producto "${p.id}" no tiene opciones de precio.`);
  }
  for (const o of p.options) {
    if (typeof o.price !== 'number' || !Number.isFinite(o.price) || o.price < 0) {
      throw new Error(`[menu] El producto "${p.id}" tiene un precio inválido en la opción "${o.label}".`);
    }
  }
}

for (const a of ADICIONES_TIPADAS) {
  if (typeof a.price !== 'number' || !Number.isFinite(a.price) || a.price < 0) {
    throw new Error(`[menu] La adición "${a.name}" tiene un precio inválido.`);
  }
  for (const c of a.cats) {
    if (!CATS_VALIDAS.has(c)) {
      throw new Error(`[menu] La adición "${a.name}" apunta a la categoría "${c}", que no existe.`);
    }
  }
}
...
export {
  MENU_TIPADO as MENU,
  ADICIONES_TIPADAS as ADICIONES,
  CATEGORIES_TIPADAS as CATEGORIES,
  COMBO_PRICE,
  COMBO_DRINKS,
  PIZZA_FLAVORS,
  BRAND,
